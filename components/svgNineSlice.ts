...
function normalize(d: string): Cmd[] {
  ...
  while (i < toks.length) {
    ...
    else if (cmd === "M") cmd = "L"; // implicit repeats of M continue as L
    else if (cmd === "m") cmd = "l";
    ...
    switch (C) {
      ...
      case "C": {
        const p = rel ? [cx + a[0], cy + a[1], cx + a[2], cy + a[3], cx + a[4], cy + a[5]] : a.slice();
      ...
      case "S": {
        const c1: [number, number] =
          prevCtl && prevCmd === "C" ? [2 * cx - prevCtl[0], 2 * cy - prevCtl[1]] : [cx, cy];
        const p = rel ? [cx + a[0], cy + a[1], cx + a[2], cy + a[3]] : a.slice();
      ...
      case "T": {
        const qc: [number, number] =
          prevQCtl && prevCmd === "Q" ? [2 * cx - prevQCtl[0], 2 * cy - prevQCtl[1]] : [cx, cy];
        const x = rel ? cx + a[0] : a[0];
        const y = rel ? cy + a[1] : a[1];
...
export function stretchPath(d: string, { origW, origH, w, h, scale = 1 }: StretchOptions): string {
  ...
  return normalize(d)
    .map(({ cmd, args }) => {
      ...
      const p: number[] = [];
      for (let j = 0; j < args.length; j += 2) p.push(r(px(args[j])), r(py(args[j + 1])));
      return `${cmd}${p.join(" ")}`;
    })
    .join(" ");
}
